...
def _make_page(text: str, page_num: int = 1, lang: str = "en") -> RawPage:
    return RawPage(doc_title="Test Doc", source_path="/tmp/test.pdf",
                   page_num=page_num, text=text, lang_hint=lang)
...
class TestEmbedder:
    ...
    def _chunk(self) -> Chunk:
        return Chunk(chunk_id=str(uuid.uuid4()), doc_title="Test",
                     source_path="/tmp/t.pdf", page_num=1, lang_hint="en",
                     text="MSME loan eligibility.", token_count=5, chunk_index=0)

    def test_upsert_called(self, mock_model, mock_col):
        from ingestion.embedder import embed_and_upsert
        with patch("ingestion.embedder._load_model", return_value=mock_model), \
