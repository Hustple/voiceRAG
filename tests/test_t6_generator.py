...
def _make_chunk(n: int = 0) -> SourceChunk:
    return SourceChunk(
        chunk_id=f"c{n}", doc_title="MSME Guide", page_num=n+1,
        chunk_text="MSME loans are available for small enterprises.", relevance_score=0.85,
    )


def _base_state(**kwargs) -> PipelineState:
    defaults = dict(
        query="What is MSME loan?", transcript="What is MSME loan?",
        lang="en", lang_hint="en", route="moderate",
        latency_map={}, self_rag_retries=0,
        crag_action="CORRECT", crag_score=0.85,
        chunks=[_make_chunk()],
    )
...
class TestGeneratorNode:
    def test_generates_answer_from_chunks(self):
        ...
        state = _base_state()
        with patch("pipeline.nodes.generator.Groq", return_value=_mock_groq("MSME loans [1] are for small businesses.")):
            result = generator_node(state)
        assert "MSME" in result["answer"]

    def test_abstain_on_incorrect_crag(self):
        from pipeline.nodes.generator import generator_node, ABSTAIN_RESPONSE_EN
    ...
    def test_abstain_hindi_on_incorrect_hindi(self):
        from pipeline.nodes.generator import generator_node, ABSTAIN_RESPONSE_HI
...
class TestQueryTextEndpoint:
    ...
    def _mock_pipeline_result(self):
        return {
            ...
            "lang": "en", "route": "moderate", "crag_action": "CORRECT",
            "crag_score": 0.85, "confidence": 0.82, "self_rag_retries": 0,
            "sources": [_make_chunk()],
            "latency_map": {"classifier": 120.0, "retrieval": 300.0, "generation": 800.0, "total": 1220.0},
