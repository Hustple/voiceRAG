...
class TestAsrNode:
    ...
    def test_audio_path_uses_whisper(self):
        ...
        with patch("pipeline.nodes.asr._transcribe_whisper", return_value="MSME loan query") as mock_w, \
    ...
    def test_hindi_audio_uses_saaras_when_key_set(self):
        ...
        with patch("pipeline.nodes.asr.settings") as mock_settings, \
             patch("pipeline.nodes.asr._transcribe_saaras", return_value="MSME प्रश्न") as mock_s, \
             patch("pipeline.nodes.asr._transcribe_whisper") as mock_w:
...
class TestVoiceEndpoint:
    ...
    def _mock_pipeline_result(self):
        return {
            ...
            "lang": "en", "route": "simple", "crag_action": "CORRECT",
            "crag_score": 0.85, "confidence": 0.82, "self_rag_retries": 0,
            "transcript": "What is MSME loan?",
            "sources": [], "latency_map": {"total": 1000.0}, "query_id": "test-id",
        }
